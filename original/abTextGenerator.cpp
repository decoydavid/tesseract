...
TextGenerator::TextGenerator(int iResolution)
{
	TextPage_s xPage;
...
vector3i TextGenerator::MapToWoorldCoord(const vector2i &vVirtualP) const
{
...
void TextGenerator::RenderString(Lattice &xLattice, const std::string &strText, const vector2i &vStart)
{
...
void TextGenerator::Update(Lattice &xLattice)
{
	Generator::Update(xLattice);
	fTimer += 0.01f;
	std::string strText = "AFRIKA BURN 2014";
...
int TextGenerator::MeasureString(const std::string &strText) const
{
	uint_8 uiStartChar = microsoftSansSerif_7ptFontInfo.startChar;
